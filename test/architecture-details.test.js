...
test('内外装資料を10企画に追記し、既存資料・本文を保全する',()=>{
 const db=new DatabaseSync(':memory:');
 for(const file of readdirSync(new URL('../migrations/',import.meta.url)).filter(x=>x.endsWith('.sql')&&!x.startsWith('0012')).sort())db.exec(readFileSync(new URL('../migrations/'+file,import.meta.url),'utf8'));
 const titles=['深海水族館','巨大クモの巣','おもちゃ工場','夢を食べるバク','ハロウィンお菓子工場','呪われたホテル','恐竜研究所','呪われた映画館','巨人の料理教室','動き出す影'].map(x=>x+'鬼ごっこ');
 const oldImages=['設計図','内装'].map(view=>({data:'data:image/png;base64,AAAA',view,note:'既存画像',source:''}));
 ...
 const sql=readFileSync(new URL('../migrations/0012_architecture_details.sql',import.meta.url),'utf8');db.exec(sql);
 ...
 for(let i=0;i<10;i++){
  ...
  assert.deepEqual(refs.stage.images.slice(0,2),oldImages);
  assert.equal(refs.stage.images.length,3);
  assert.equal(refs.stage.images[2].view,'細部');
  assert.match(refs.stage.brief,/^現在の制作条件を保持\n【内外装の具体化 v2】/);
  assert.match(refs.stage.brief,/ゲーム内未確認/);
 }
 assert.equal(db.prepare('SELECT count(*) AS n FROM architecture_reference_backup').get().n,10);
