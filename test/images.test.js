...
test('画像3種の保存・再読込・差し替え・削除と既存投稿の互換性',async()=>{
 ...
 assert.equal((await call('/ideas','POST',{...note,stage_image:picture,monster_image:picture,mission_image:picture,runner_image:picture,runner:"水族館の飼育員3人"})).status,201);
 const list=await (await call('/ideas')).json(),id=list.items[0].id,path='/ideas/'+id;
 assert.equal(list.items[0].stage_image,undefined);
 assert.equal(list.items[0].runner,"水族館の飼育員3人");
 ...
 assert.equal((await (await call(path+'/images')).json()).images.runner_image,picture);
 ...
 assert.equal((await (await call(path+'/images')).json()).images.monster_image,picture);
 assert.equal((await call(path,'PUT',{...note,runner:'探検家3人',runner_image:''})).status,200);
 assert.equal((await (await call('/ideas')).json()).items[0].runner,'探検家3人');
 assert.equal((await (await call(path+'/images')).json()).images.runner_image,'');
