...
function renderProduction(id){
 ...
 (p?.state==='停止'?'<p>以前の依頼先が停止したことを確認してから再開してください。旧成果物は制作履歴に残し、新しい台本から進めます。</p><button type="button" data-production="restart">台本から制作を再開</button>':'')+
 ...
 host.querySelectorAll('[data-production]').forEach(b=>b.onclick=()=>saveProduction(id,b.dataset.production,{note:b.dataset.production==='cancel'?'メンバーが受け渡しを停止':b.dataset.production==='restart'?'担当と停止状況を確認し、台本から制作を再開':''}));
